...
import { armOf, bugHuntRuns, runText } from './bench-subagents-read.mjs'
...
const header = ['arm', 'n', 'found~', 'found avg', 'range', 'turns', 'tokens', 'sec', 'subAgents']
...
for (const run of scored) {
  console.log(
    `  arm=${run.arm.padEnd(3)} ${run.ended.padEnd(8)} found=${String(run.found).padStart(2)}/12` +
      ` turns=${String(run.turns).padStart(2)} tokens=${String(run.tokens).padStart(6)}` +
