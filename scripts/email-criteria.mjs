 ...
 * accumulate unseen. The same was true of the scoring loop, which this file
 * also carried its own copy of: it never learned the `inconclusive` state, so a
 * turn the runtime ended on a context limit -- no model text at all -- was
 * scored as a bad answer. `handles-a-bad-id` reported a consistent failure on
 * qwen27b across both passes on 2026-09-05 for exactly that, which reads as
 * "the model invented a message" when the model never got to speak.
 */

const { logPath, flags } = graderArgs('email-criteria.mjs')

gradeLog({
  logPath,
  flags,
  expectedTurns: 7,
  buildCriteria: ({ turns, reply, calls }) => [
    {
