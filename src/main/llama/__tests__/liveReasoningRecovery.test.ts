 ...
 *
 * What it measured on Qwen3.6-27B Q4_K_M, 32,768 context, before the fix: a
 * 900-token round produced 3,477 characters of reasoning, zero visible text,
 * zero tool calls, and `finish_reason: "length"` — the dead round that used to
 * end the whole turn and send the bounded runner into a fresh cycle that
 * restarted the same task. See `reasoningOverrun.ts`.
 *
...
const mocks = vi.hoisted(() => ({ toolCalls: [] as Array<{ name: string; args: unknown }> }))
...
describe.skipIf(!BASE_URL)('live reasoning-overrun recovery', () => {
  it('reaches a real tool call on a request that reasons past its budget', async () => {
    ...
    const outcome = await service.generate({
      ...
      // Small enough that a thorough deliberation outruns it, which is the
      // whole scenario. At a realistic ceiling the budget only bites on a
      // genuine runaway — the measured failure was 3x over.
