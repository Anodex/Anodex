 ...
 * comet's current phase. The tone -> phase mapping happens synchronously
 * during render (not in an effect) so an already-in-flight arrival can't
 * lose a frame to a plain-colored dot before the flare kicks in — this
 * matters here because ModelStatusMenu swaps which JSX branch (and which
 * dot element) is mounted right as loading resolves to ready.
 */
