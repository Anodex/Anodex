...
import { stripLeakedChannelTokens, findPotentialToolCallTextStart } from '../toolCallText'
import {
  DEEPSEEK_CALL_BEGIN,
...
describe('fabricated tool outputs in visible text', () => {
  it('drops an invented result block, content and all', () => {
    ...
    expect(stripLeakedChannelTokens(text)).toBe('I will continue from where I left off.')
  })
  ...
  it('drops an unterminated block rather than leaving the worst case on screen', () => {
    ...
    expect(stripLeakedChannelTokens(text)).toBe('Reading it now.')
  })
  ...
  it('removes a stray marker without eating the prose around it', () => {
    ...
    expect(stripLeakedChannelTokens(text)).toBe('Done.\nThe imports are global now.')
  })

  it('leaves ordinary text alone', () => {
    const text = 'I read the file and it still uses ES module imports.'
    expect(stripLeakedChannelTokens(text)).toBe(text)
  })
